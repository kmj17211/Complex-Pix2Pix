...
from torch.nn.functional import leaky_relu, relu, max_pool2d, avg_pool2d, dropout, dropout2d, interpolate
...
def complex_leaky_relu(input, negative_slope = 0.1):
    return leaky_relu(input.real, negative_slope = negative_slope).type(torch.complex64)\
        +1j*leaky_relu(input.imag, negative_slope = negative_slope).type(torch.complex64)
