...
from .complexFunctions import complex_leaky_relu, complex_relu, complex_max_pool2d, complex_avg_pool2d
from .complexFunctions import complex_dropout, complex_dropout2d
from .complexFunctions import complex_sigmoid, complex_tanh
from .complexFunctions import complex_opposite
...
class ComplexLeakyReLU(Module):
    ...
    def __init__(self, negative_slobe = 0.1):
        super(ComplexLeakyReLU, self).__init__()
        self.negative_slobe = negative_slobe

    def forward(self, input):
        return complex_leaky_relu(input, self.negative_slobe)
...
class _ComplexBatchNorm(Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        if self.affine:
            ...
            self.weight = Parameter(torch.Tensor(num_features,2))
...
class ComplexBatchNorm2d(_ComplexBatchNorm):
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            input = (self.weight[None,:,0,None,None]*input.real+self.bias[None,:,0,None,None]).type(torch.complex64) \
                    +1j*(self.weight[None,:,1,None,None]*input.imag+self.bias[None,:,1,None,None]).type(torch.complex64)
...
class ComplexInstanceNorm2d(Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=False):
        ...
        if self.affine:
            self.gamma_r = Parameter(torch.ones(num_features))
            self.beta_r = Parameter(torch.zeros(num_features))
            self.gamma_i = Parameter(torch.ones(num_features))
            self.beta_i = Parameter(torch.zeros(num_features))
        else:
            self.register_parameter('gamma_r', None)
            self.register_parameter('beta_r', None)
            self.register_parameter('gamma_i', None)
            self.register_parameter('beta_i', None)


    def forward(self, input):
        assert input.dim() == 4, 'ComplexInstanceNorm2d only supports 4D input.'
        ...
        # Compute the mean and variance for each channel separately
        input_r_mean = input_r.mean(dim=(2, 3), keepdim=True)
        input_i_mean = input_i.mean(dim=(2, 3), keepdim=True)

        input_r = input_r - input_r_mean
        input_i = input_i - input_i_mean

        n = input.numel() / (batch_size * num_channels)
        Crr = 1./n*input_r.pow(2).sum(dim=[2,3], keepdim = True)+self.eps
        Cii = 1./n*input_i.pow(2).sum(dim=[2,3], keepdim = True)+self.eps
        Cri = (input_r.mul(input_i)).mean(dim=[2,3], keepdim = True)
        ...
        input = (Rrr*input_r+Rri*input_i).type(torch.complex64) + 1j*(Rii*input_i+Rri*input_r).type(torch.complex64)

        if self.affine:
            input = (self.gamma_r[None,:,None,None]*input.real + self.beta_r[None,:,None,None].type(torch.complex64)) \
                    +1j*(self.gamma_i[None,:,None,None]*input.imag + self.beta_i[None,:,None,None].type(torch.complex64))
